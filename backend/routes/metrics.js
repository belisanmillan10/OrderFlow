...
// GET /api/metrics/local - metricas generales del local
router.get('/local', async (req, res) => {
  try {
    ...
    const prodsRes = await pool.query('SELECT available FROM products');
    const unavailPct = prodsRes.rows.length
      ? Math.round((prodsRes.rows.filter((p) => !p.available).length / prodsRes.rows.length) * 100)
      : 0;

    // recompra: clientes con mas de 1 pedido
    const repeatRes = await pool.query(`
      SELECT customer_name, COUNT(*) AS cnt FROM orders GROUP BY customer_name
    `);
    ...
    const slotsRes = await pool.query('SELECT max_capacity, used_capacity FROM time_slots');
...
// GET /api/metrics/top-products - productos mas vendidos
...
// GET /api/metrics/by-channel - pedidos y ventas por canal
...
// GET /api/metrics/operational - rechazados, reasignados, etc
router.get('/operational', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM operational_metrics WHERE id = 1');
